...
export class CollabTextNode {
  ...
  spliceText(index: number, delCount: number, newText: string): void {
    const collabElementNode = this._parent;
    const loroText = collabElementNode._loroText;
    const offset = this.getOffset() + 1 + index;

    if (delCount !== 0) {
      loroText.delete(offset, delCount);
    }

    if (newText !== '') {
      loroText.insert(offset, newText);
    }
  }
  ...
  syncPropertiesAndTextFromLoro(
    ...
    invariant(
      lexicalNode !== null,
      'syncPropertiesAndTextFromLoro: could not find text node',
    );

    $syncPropertiesFromLoro(binding, this._map, lexicalNode, keysChanged);
