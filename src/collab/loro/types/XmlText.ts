...
export class XmlText {
  ...
  private _validateText(): void {
    try {
      ...
      console.debug(`XmlText initialized - ID: ${id}, Length: ${length}`);
    } catch (error) {
  ...
  insert(offset: number, text: string, attributes?: TextAttributes): void {
    ...
    console.debug(`[XmlText] insert called with offset: ${offset}, text: "${text}", attributes:`, attributes);
    ...
    try {
      ...
      console.debug(`[XmlText] About to call LoroText.insert(${offset}, "${text}")`)
      this._text.insert(offset, text);
      console.debug(`[XmlText] LoroText.insert completed successfully`)
      ...
      console.debug(`[XmlText] Successfully inserted text at offset ${offset}`);
      
      // Commit the transaction to trigger Loro's event system
      console.debug(`[XmlText] *** COMMITTING LORO TRANSACTION ***`);
      try {
        this._doc.commit();
        console.debug(`[XmlText] *** COMMIT COMPLETED SUCCESSFULLY ***`);
      } catch (error) {
  ...
  insertEmbed(offset: number, object: any, attributes?: TextAttributes): void {
    console.debug(`insertEmbed called - offset: ${offset}, object type: ${object?.constructor?.name}`);
    ...
    try {
      ...
      console.debug(`Successfully inserted embed placeholder at offset ${offset}, stored as ${embedKey}`);
  ...
  format(index: number, length: number, attributes: TextAttributes): void {
    ...
    console.debug(`Applied formatting to range [${index}, ${index + length}):`, attributes);
  ...
  delete(offset: number, length: number): void {
    ...
    if (offset >= 0 && offset < textLength) {
      ...
      console.debug(`[XmlText] About to delete ${actualLength} characters at offset ${offset}`);
      this._text.delete(offset, actualLength);
      console.debug(`[XmlText] Delete completed successfully`);
      
      // MANUAL FIX: Trigger document update after deletion
      try {
        const update = this._doc.export({ mode: 'update' });
        if (update.length > 0) {
          console.debug(`[XmlText] Manually triggering document update after delete - size: ${update.length}`);
