...
export function LoroCollaborationPluginV2({
  ...
  }, [editor, provider, id, loroDocMap, doc]);
...
function LoroCollaborationCursors({
  ...
  useEffect(() => {
    ...
    const handleWelcome = (data: any) => {
      ...
      if (data.peers && onAwarenessChange) {
        ...
        // Filter out any stale peer entries that might not be properly cleaned up
        ...
        const awarenessData = validPeers.map((peer: any) => ({
          ...
          isCurrentUser: peer.isCurrentUser || peer.websocketClientId === provider.clientId
        }));
      ...
      // Also set WebSocket client ID from welcome message (use WebSocket client ID for connection tracking)
      if (data.clientId && onWebSocketClientIdChange) {
        onWebSocketClientIdChange(data.clientId);
      }
    };
    ...
    const handlePeerUpdate = (data: any) => {
      ...
      if (data.peers && onAwarenessChange) {
        // Filter out any stale peer entries and ensure no duplicates based on browser session
        const validPeers = data.peers.filter((peer: any) => peer.id && peer.websocketClientId);
        ...
        validPeers.forEach((peer: any) => {
          ...
          const isCurrentUser = peer.isCurrentUser || peer.websocketClientId === provider.clientId;
  ...
  }, [provider, onWebSocketClientIdChange, onAwarenessChange]);
